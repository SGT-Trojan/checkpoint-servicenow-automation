...
PATTERNS = (
    ...
    (
        re.compile(r"/(?:home/[a-z_][a-z0-9_-]*|Users/[^/\s]+)/"),
        "absolute home-directory path",
    ),
    (re.compile(r"otpauth://", re.I), "TOTP URI"),
    (
        re.compile(r"-----BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY-----"),
        "private key",
    ),
    (
        re.compile(r"(?i)\b(?:authToken|clientSessionId|fwmSessionId)\s*[:=]\s*[\"']?(?!<REDACTED>|null|none)[A-Za-z0-9+/=_-]{8,}"),
        "session token",
    ),
...
# The pattern table necessarily contains examples that it is designed to find.
