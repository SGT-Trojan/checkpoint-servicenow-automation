...
| `direct_package_step_from_activity.py` | `--activity-plan-file`, `--reports-dir`, `--phase`, `--step` | `--username`; `--timeout`; `--auto-reboot-grace`; `--explicit-reboot-fallback` | Requires `--execute` to install/remove/reboot |
