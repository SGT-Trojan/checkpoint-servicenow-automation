...
| `deployment_backend` | Backend playbooks | `cdt`, `api`, or `direct`; selecting a value does not itself authorize execution |
...
| `generate_cdt_candidates_from_activity.py` | `--activity-plan-file` | `--step`; `--plan-path`; `--candidates-path`; `--target-policy standby|active`; `--target-ip`; `--username` | Generates/re-writes plan and candidate files on MDS; does not deploy |
...
| `direct_package_step_from_activity.py` | `--activity-plan-file`, `--reports-dir`, `--phase`, `--step` | `--username`; `--timeout`; `--auto-reboot-grace`; `--explicit-reboot-fallback` | Requires `--execute`; non-interactive commands require a captured exit status, and interactive removal requires exact package absence after reboot |
...
| `major_mvc_from_activity.py` | `--activity-plan-file`, `--reports-dir`, `--phase mvc-on|mvc-off` | `--username` | Changes MVC state |
...
| `10_cdt_generate_candidates.yml` | `activity_plan_file`, `activity_plan` | `step`; `phase`; `cdt_target_policy standby|active`; `chg_number` | Generates one-member controlled candidate files |
...
| `20_cdt_execute_guarded.yml` | `activity_plan_file`, `activity_plan`, `checkpoint_execute_upgrade=true` | `step`; `phase`; `checkpoint_cdt_execute_timeout`; `chg_number` | Mutating CDT deployment |
